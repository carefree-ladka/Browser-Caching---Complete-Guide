...
**DevTools Indicator:**  
Network tab → Size column: `(from memory cache)`
...
**DevTools Indicator:**  
Network tab → Size column: `(from disk cache)`
...
4. When code changes → hash changes → new filename → automatic cache invalidation
...
- Browser cache shorter → fresher content for repeat visitors
- CDN cache longer → reduced origin server load
...
1. First user request → CDN fetches from origin, caches for 1 hour
2. Next user request → CDN serves from edge cache (very fast)
3. After 1 hour → CDN revalidates with origin
...
### ❓ Why not cache HTML aggressively?
...
### ❓ What improves LCP (Largest Contentful Paint) the most?
...
├─ Yes → max-age=1y, immutable
└─ No → Does it change often?
    ├─ Yes → no-cache or no-store
    └─ No → max-age=30d (moderate)
```
...
- 🔥 **Caching vs LocalStorage vs IndexedDB** - Client-side storage comparison
- 🔥 **Browser Cache + React SSR (Next.js)** - Server-side rendering caching strategies
- 🔥 **HTTP/2 Push and Caching** - Interaction between server push and cache
- 🔥 **Cache Partitioning** - Privacy-focused caching changes in modern browsers
